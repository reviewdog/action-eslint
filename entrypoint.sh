...
if [ ! -f "$(npm bin --prefix $INPUT_WORKDIR)/eslint" ]; then
  npm install --prefix $INPUT_WORKDIR
fi

$(npm bin --prefix $INPUT_WORKDIR)/eslint --version

if [ "${INPUT_REPORTER}" == 'github-pr-review' ]; then
  # Use jq and github-pr-review reporter to format result to include link to rule page.
  $(npm bin)/eslint --resolve-plugins-relative-to $INPUT_WORKDIR -f="json" ${INPUT_ESLINT_FLAGS:-'.'} \
  ...
  $(npm bin)/eslint --resolve-plugins-relative-to $INPUT_WORKDIR -f="stylish" ${INPUT_ESLINT_FLAGS:-'.'} \
